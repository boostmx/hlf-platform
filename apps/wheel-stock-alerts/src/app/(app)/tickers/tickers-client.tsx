...
import { TrendingUp, Search, Check, Plus, X, EyeOff, Eye, Trash2, Loader2, AlertCircle } from "lucide-react";
...
export function TickersClient({
  ...
  const [validation, setValidation] = useState<{ state: "idle" | "loading" | "valid" | "error"; message?: string }>({ state: "idle" });
  ...
  async function toggleSubscription(tickerId: string, symbol: string) {
    ...
    setSubscribedIds(prev => {
      ...
      if (isSubscribed) { next.delete(tickerId); toast.success(`Unsubscribed from ${symbol}.`); }
      else { next.add(tickerId); toast.success(`Subscribed to ${symbol}.`); }
      return next;
    });
  }
  ...
  async function toggleHide(tickerId: string, symbol: string) {
    ...
    setHiddenIds(prev => {
      const next = new Set(prev);
      if (isHidden) { next.delete(tickerId); toast.success(`${symbol} restored.`); }
  ...
  async function validateSymbol(symbol: string) {
    if (!symbol) { setValidation({ state: "idle" }); return; }
  ...
  async function handleDeleteTicker(tickerId: string, symbol: string) {
    ...
    setDeleting(null);
    if (!res.ok) { toast.error("Failed to delete ticker."); return; }
    setTickers(prev => prev.filter(t => t.id !== tickerId));
    toast.success(`${symbol} removed.`);
  }

  async function handleAddTicker() {
    if (!addForm.symbol || !addForm.name) { toast.error("Symbol and name are required."); return; }
    setAdding(true);
  ...
  const curatedVisible = tickers.filter(t => t.createdBy === null && !hiddenIds.has(t.id) && matchesSearch(t));
  const curatedHidden = tickers.filter(t => t.createdBy === null && hiddenIds.has(t.id) && matchesSearch(t));
  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <Button variant="outline" size="sm" className="gap-2 shrink-0" onClick={() => { setShowAdd(v => !v); setValidation({ state: "idle" }); setAddForm({ symbol: "", name: "", sector: "" }); }}>
          ...
          <div className="grid sm:grid-cols-3 gap-3">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <Input
                  ...
                  onChange={e => { setAddForm(p => ({ ...p, symbol: e.target.value.toUpperCase() })); setValidation({ state: "idle" }); }}
                  onBlur={e => validateSymbol(e.target.value.trim())}
                  placeholder="AAPL"
                  className={cn(
                    validation.state === "valid" && "border-emerald-500 focus-visible:ring-emerald-500/30",
                    validation.state === "error" && "border-destructive focus-visible:ring-destructive/30"
                  )}
                />
                <div className="absolute right-2.5 top-1/2 -translate-y-1/2">
                  {validation.state === "loading" && <Loader2 className="h-3.5 w-3.5 animate-spin text-muted-foreground" />}
                  {validation.state === "valid"   && <Check className="h-3.5 w-3.5 text-emerald-500" />}
                  {validation.state === "error"   && <AlertCircle className="h-3.5 w-3.5 text-destructive" />}
                </div>
              </div>
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder={validation.state === "loading" ? "Looking up…" : "Auto-filled from symbol"}
              />
            </div>
            <div className="space-y-1.5">
              <Label>Sector <span className="text-muted-foreground font-normal">(optional)</span></Label>
              <Input value={addForm.sector} onChange={e => setAddForm(p => ({ ...p, sector: e.target.value }))} placeholder="Technology" />
            </div>
          </div>
          <Button size="sm" onClick={handleAddTicker} disabled={adding || validation.state !== "valid"} className="gap-2">
      ...
      <div className="space-y-3">
        <div className="relative">
          ...
          <Input placeholder="Search symbol, name, sector…" className="pl-9" value={search} onChange={e => setSearch(e.target.value)} />
        </div>
      ...
      {totalVisible === 0 && curatedHidden.length === 0 ? (
        ...
        <>
          ...
          {/* Hidden tickers — expandable at bottom */}
          {curatedHidden.length > 0 && (
            <div>
              ...
              >
                ...
                {showHidden ? "Hide" : "Show"} {curatedHidden.length} hidden ticker{curatedHidden.length !== 1 ? "s" : ""}
              </button>
              {showHidden && (
                <div className="mt-3 grid sm:grid-cols-2 lg:grid-cols-3 gap-3 opacity-60">
                  {curatedHidden.map(ticker => (
                    <div key={ticker.id} className="border border-border rounded-lg p-3 bg-card flex items-center justify-between gap-3">
...
function TickerSection({
  label, description, tickers, subscribedIds, loading, hiding, deleting, onToggle, onHide, onDelete, showHideButton,
}: {
  label: string;
  description: string;
  tickers: Ticker[];
    ...
    <div className="space-y-3">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {tickers.map(ticker => {
          ...
          const sectorClass = ticker.sector ? (SECTOR_COLORS[ticker.sector] ?? "bg-slate-500/15 text-slate-400 border-slate-500/25") : null;
          return (
            <div
              key={ticker.id}
              className={cn(
            ...
            >
              ...
              <div className="flex items-center justify-between gap-2">
                {sectorClass ? (
                  <span className={cn("inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium border", sectorClass)}>
                    {ticker.sector}
                  </span>
                ) : <span />}
                <Button
                  ...
                  className={cn("gap-1.5 shrink-0", subscribed && "border-primary/40 text-primary")}
                >
                  {loading === ticker.id ? "…" : subscribed ? <><Check className="h-3 w-3" />Subscribed</> : "Subscribe"}
                </Button>
              </div>
            </div>
          );
