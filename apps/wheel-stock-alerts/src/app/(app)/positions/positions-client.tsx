...
import { TrendingDown, TrendingUp, BarChart2, Clock, ExternalLink, Plus, X, Trash2, CheckSquare } from "lucide-react";
...
import type { WheelOpenPositions, WheelOpenTrade, WheelOpenStockLot } from "@/lib/wheel-tracker-client";
...
const TRACKER_TRADE_META: Record<string, { label: string; icon: React.ElementType; color: string }> = {
...
function TrackerTradeRow({ trade }: { trade: WheelOpenTrade }) {
  const meta = TRACKER_TRADE_META[trade.type] ?? { label: trade.type, icon: BarChart2, color: "bg-muted text-muted-foreground border-border" };
    ...
    <div className="border border-border rounded-lg p-4 bg-card flex items-start gap-4">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap mb-1">
          ...
          <span className={cn("inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium border", meta.color)}>
            <Icon className="h-3 w-3" />{meta.label}
          </span>
          <span className="text-sm font-medium">${trade.strikePrice.toFixed(2)} strike</span>
          <span className="text-xs text-muted-foreground">${trade.contractPrice.toFixed(2)} premium × {trade.contractsOpen * 100} shares</span>
        </div>
        <div className="flex items-center gap-4 text-xs text-muted-foreground flex-wrap">
          <span className={getDteColor(dte)}>{dte > 0 ? `${dte} DTE` : dte === 0 ? "Expires today" : "Expired"}</span>
          ...
          {trade.contractsOpen !== trade.contractsInitial && (
            <span className="text-amber-500">{trade.contractsOpen}/{trade.contractsInitial} contracts open</span>
          )}
        </div>
      </div>
    </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap mb-1">
          ...
          <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium border bg-violet-500/15 text-violet-400 border-violet-500/25">
            <BarChart2 className="h-3 w-3" />Stock
          </span>
          <span className="text-sm font-medium">{lot.shares} shares</span>
          <span className="text-xs text-muted-foreground">avg cost ${lot.avgCost.toFixed(2)}</span>
        </div>
        <div className="flex items-center gap-4 text-xs text-muted-foreground flex-wrap">
...
function ManualPositionRow({ position, onClose, onDelete }: {
  ...
  return (
    <div className="border border-border rounded-lg p-4 bg-card flex items-start gap-4">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap mb-1">
          ...
          {position.positionType !== "STOCK" && position.premium && (
            <span className="text-xs text-muted-foreground">${position.premium.toFixed(2)} premium × {position.contracts * 100} shares</span>
          )}
          ...
          {position.positionType === "STOCK" && position.entryPrice && (
            <span className="text-xs text-muted-foreground">entry ${position.entryPrice.toFixed(2)}</span>
          )}
        </div>
        <div className="flex items-center gap-4 text-xs text-muted-foreground flex-wrap">
          {dte !== null && (
      ...
      <div className="flex items-center gap-1 shrink-0">
        <Button variant="ghost" size="icon-sm" className="text-muted-foreground hover:text-foreground" onClick={() => onClose(position.id)} title="Mark closed">
          <CheckSquare className="h-4 w-4" />
        </Button>
        <Button variant="ghost" size="icon-sm" className="text-muted-foreground hover:text-rose-500" onClick={() => onDelete(position.id)} title="Delete">
...
const EMPTY_FORM: FormState = {
  tickerId: "", positionType: "CSP", contracts: "1",
  strikePrice: "", premium: "", shares: "", entryPrice: "",
  expirationDate: "", notes: "",
};

function AddPositionModal({ tickers, onClose, onSaved }: {
  tickers: Ticker[];
  onClose: () => void;
  onSaved: () => void;
}) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    if (!form.tickerId) { toast.error("Select a ticker."); return; }
    setSaving(true);
    ...
    setSaving(false);
    if (res.ok) { toast.success("Position added."); onSaved(); }
    else { const d = await res.json(); toast.error(d.error ?? "Failed to add position."); }
  }
      ...
      <div className="relative bg-background border border-border rounded-xl shadow-xl w-full max-w-md p-6 space-y-5">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {isOption ? (
              <>
                ...
                <div className="space-y-1.5">
                  ...
                  <Input type="number" step="0.01" placeholder="0.00" value={form.strikePrice} onChange={(e) => set("strikePrice", e.target.value)} />
                </div>
                <div className="space-y-1.5">
                  <Label>Premium</Label>
                  <Input type="number" step="0.01" placeholder="0.00" value={form.premium} onChange={(e) => set("premium", e.target.value)} />
                </div>
                <div className="space-y-1.5">
                  <Label>Expiration</Label>
                  <DatePicker value={form.expirationDate} onChange={(v) => set("expirationDate", v)} />
                </div>
              </>
            ) : (
              <>
                <div className="space-y-1.5">
                  <Label>Shares</Label>
                  <Input type="number" min={1} placeholder="100" value={form.shares} onChange={(e) => set("shares", e.target.value)} />
                </div>
                <div className="space-y-1.5">
                  <Label>Entry Price</Label>
                  <Input type="number" step="0.01" placeholder="0.00" value={form.entryPrice} onChange={(e) => set("entryPrice", e.target.value)} />
                </div>
              </>
            )}
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <Input placeholder="e.g. hedge position" value={form.notes} onChange={(e) => set("notes", e.target.value)} />
          </div>

          <div className="flex gap-2 pt-1">
            <Button type="button" variant="outline" className="flex-1" onClick={onClose}>Cancel</Button>
            <Button type="submit" className="flex-1" disabled={saving}>{saving ? "Saving…" : "Add Position"}</Button>
          </div>
        </form>
      </div>
    </div>
...
export function PositionsClient({
  ...
  async function handleClose(id: string) {
    const res = await fetch(`/api/positions/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ close: true }) });
    if (res.ok) { toast.success("Position marked closed."); router.refresh(); }
    else toast.error("Failed to close position.");
  }

  async function handleDelete(id: string) {
    const res = await fetch(`/api/positions/${id}`, { method: "DELETE" });
    if (res.ok) { toast.success("Position deleted."); router.refresh(); }
    else toast.error("Failed to delete position.");
  }

  const TabButton = ({ value, label, count }: { value: "tracker" | "manual"; label: string; count: number }) => (
    <button
  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <a
          href={process.env.NEXT_PUBLIC_WHEEL_TRACKER_URL ?? "https://wheel.hlfinancialstrategies.com"}
          target="_blank"
      ...
      {tab === "tracker" && (
        !isTrackerConfigured ? (
          <div className="border border-border rounded-lg p-10 text-center text-sm text-muted-foreground">
            ...
            <p className="text-xs">Set <code className="font-mono text-xs">WHEEL_TRACKER_URL</code> and <code className="font-mono text-xs">INTERNAL_API_KEY</code> to enable live position sync.</p>
          </div>
        ) : trackerCount === 0 ? (
          <div className="border border-border rounded-lg p-10 text-center text-sm text-muted-foreground">
            <Clock className="h-8 w-8 mx-auto mb-3 opacity-20" />
          ...
          <div className="space-y-6">
            {trackerPositions.trades.length > 0 && (
              <div>
                <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground mb-3">Options ({trackerPositions.trades.length})</h2>
                <div className="space-y-2">
                  {trackerPositions.trades.map((t) => <TrackerTradeRow key={t.id} trade={t} />)}
                </div>
              </div>
            )}
            {trackerPositions.stockLots.length > 0 && (
              <div>
                <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground mb-3">Stock Lots ({trackerPositions.stockLots.length})</h2>
                <div className="space-y-2">
                  {trackerPositions.stockLots.map((l) => <TrackerLotRow key={l.id} lot={l} />)}
                </div>
              </div>
            )}
          </div>
        ...
        <div className="space-y-4">
          ...
          {manualCount === 0 ? (
            <div className="border border-border rounded-lg p-10 text-center text-sm text-muted-foreground">
              ...
              <p className="text-xs mt-1">Add positions here to receive exit alerts for trades outside Wheel Tracker.</p>
            </div>
          ) : (
            <div className="space-y-2">
              {initialManual.map((p) => (
                <ManualPositionRow key={p.id} position={p} onClose={handleClose} onDelete={handleDelete} />
              ))}
            </div>
          )}
        </div>
