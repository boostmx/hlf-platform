...
export function computeSignals(bars: Bar[]): Signals {
  if (bars.length === 0) {
    return { rsi14: null, sma50: null, sma200: null, supports: [], resistances: [], avgVolume20: null, latestVolume: null, latestClose: null };
  }
...
export function evaluateTriggers(
  ...
  const { rsi14, sma50, sma200: _sma200, supports, resistances, latestClose, latestVolume, avgVolume20 } = signals;

  if (latestClose === null) return triggers;

  // CSP opportunity: oversold RSI + price near/above support
  if (rsi14 !== null && rsi14 <= thresholds.rsiOversold) {
    ...
    if (nearSupport || supports.length === 0) {
      triggers.push("CSP_OPPORTUNITY");
    }
  }

  // CC opportunity: overbought RSI + price near resistance
  if (rsi14 !== null && rsi14 >= thresholds.rsiOverbought) {
    ...
    if (nearResistance || resistances.length === 0) {
      triggers.push("CC_OPPORTUNITY");
    }
  }

  // Support break: price dropped below nearest support
  if (supports.length > 0) {
    const nearestSupport = Math.max(...supports);
    if (latestClose < nearestSupport * 0.99) {
      triggers.push("SUPPORT_BREAK");
    }
  }

  // Resistance break: price moved above nearest resistance
  if (resistances.length > 0) {
    const nearestResistance = Math.min(...resistances);
    if (latestClose > nearestResistance * 1.01) {
      triggers.push("RESISTANCE_BREAK");
    }
  }

  // Volume surge
  if (latestVolume !== null && avgVolume20 !== null && latestVolume > avgVolume20 * thresholds.volumeSurgeMultiplier) {
    triggers.push("VOLUME_SURGE");
  }

  // SMA crosses (require previous signals to compare)
  if (
    prevSignals !== null &&
