...
function SignalPills({ signals }: { signals: Signals }) {
  ...
  if (signals.rsi14 !== null) pills.push({ label: "RSI", value: String(signals.rsi14) });
  if (signals.latestClose !== null)
    pills.push({ label: "Price", value: `$${signals.latestClose.toFixed(2)}` });
  if (signals.supports.length > 0)
    pills.push({
      label: "Support",
      value: signals.supports.map((s) => `$${s.toFixed(2)}`).join(" / "),
    });
  if (signals.resistances.length > 0)
    pills.push({
      label: "Resistance",
      value: signals.resistances.map((r) => `$${r.toFixed(2)}`).join(" / "),
    });
  ...
  if (signals.latestVolume !== null && signals.avgVolume20 !== null)
    pills.push({
      label: "Vol",
      value: `${(signals.latestVolume / 1_000_000).toFixed(1)}M (${(
  ...
  return (
    <div className="flex flex-wrap gap-1.5 mt-2">
      {pills.map(({ label, value }) => (
        <span
          key={label}
          className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-muted text-[10px] font-medium"
        >
    ...
    <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          All alerts sent to you, newest first.
        </p>
      </div>
