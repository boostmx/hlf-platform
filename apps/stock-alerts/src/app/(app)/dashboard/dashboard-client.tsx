...
export function DashboardClient({
  ...
  const channelCount = new Set(
    recentAlerts.flatMap((a) => a.channel.split(",").filter(Boolean)),
  ).size;
    ...
    <div className="p-4 md:p-6 max-w-6xl mx-auto space-y-6 md:space-y-8">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Your recent alerts and subscribed tickers.
        </p>
      </div>
          ...
          {subscriptions.length === 0 ? (
            ...
            <div className="space-y-2">
              {subscriptions.map(({ ticker }) => (
                <div
                  key={ticker.id}
                  className="border border-border rounded-lg p-3 flex items-center justify-between bg-card"
                >
