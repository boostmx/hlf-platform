...
export function AdminClient({ tickers: initialTickers }: { tickers: Ticker[] }) {
  ...
  async function addTicker() {
    if (!symbol || !name) { toast.error("Symbol and name are required."); return; }
    setAdding(true);
  ...
  async function toggleApproved(id: string, current: boolean) {
    ...
    setTickers((prev) => prev.map((t) => t.id === id ? { ...t, isApproved: !current } : t));
  }
  ...
  return (
    <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Add and manage the approved wheel universe.</p>
      </div>
      ...
      <div>
        ...
        <div className="space-y-2">
          {tickers.map((ticker) => (
            <div key={ticker.id} className="border border-border rounded-lg p-3 flex items-center gap-4">
              <div className="flex-1">
                <span className="font-medium">{ticker.symbol}</span>
                <span className="text-sm text-muted-foreground ml-2">{ticker.name}</span>
                {ticker.sector && <Badge variant="secondary" className="ml-2 text-xs">{ticker.sector}</Badge>}
              </div>
