...
export async function POST(req: NextRequest) {
  ...
  const { tickerId, positionType, contracts, strikePrice, premium, shares, entryPrice, expirationDate, notes } = parsed.data;

  const ticker = await prisma.ticker.findUnique({ where: { id: tickerId }, select: { id: true } });
