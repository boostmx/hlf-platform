...
// Keep single-alert payload for backwards compat with manual /api/cron/scan
...
// ─── Single alert dispatch (manual scan / backwards compat) ──────────────────
...
export async function sendAlerts(payload: AlertPayload): Promise<string[]> {
  return sendDigest({
    ...
    alerts: [{ symbol: payload.symbol, triggerType: payload.triggerType, signals: payload.signals, message: payload.message }],
  });
}
...
function signalRows(signals: Signals): string {
  ...
  if (signals.rsi14 !== null)
    rows.push(pill("RSI (14)", String(signals.rsi14)));
  if (signals.latestClose !== null)
    rows.push(pill("Price", `$${signals.latestClose.toFixed(2)}`));
  ...
  if (signals.resistances.length > 0)
    rows.push(pill("Resistance", signals.resistances.map(r => `$${r.toFixed(2)}`).join(" / ")));
  if (signals.sma50 !== null)
    rows.push(pill("50 SMA", `$${signals.sma50.toFixed(2)}`));
  if (signals.sma200 !== null)
    rows.push(pill("200 SMA", `$${signals.sma200.toFixed(2)}`));
  if (signals.latestVolume !== null && signals.avgVolume20 !== null)
    rows.push(pill("Volume", `${(signals.latestVolume / 1_000_000).toFixed(1)}M (${(signals.latestVolume / signals.avgVolume20).toFixed(1)}x avg)`));
  return rows.join("");
}
...
async function sendDigestEmail(payload: DigestPayload) {
  ...
  const subject = count === 1
    ? `HLF Wheel Alerts — ${payload.alerts[0].symbol} · ${TRIGGER_LABELS[payload.alerts[0].triggerType]}`
    : `HLF Wheel Alerts — ${count} signals today (${tickers})`;
...
<body style="margin:0;padding:0;background:#0a0914;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
  <div style="max-width:600px;margin:0 auto;padding:24px 16px;">
    ...
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;padding-bottom:20px;border-bottom:1px solid #2d2a42;">
      <div style="width:36px;height:36px;background:#7c3aed22;border-radius:8px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
        <span style="font-size:18px;">🔔</span>
      </div>
      <div>
        <p style="margin:0;font-size:15px;font-weight:700;color:#e2e0f0;">HLF Wheel Alerts</p>
        <p style="margin:0;font-size:12px;color:#8b8aa0;">${date}</p>
      </div>
      <div style="margin-left:auto;background:#7c3aed22;border:1px solid #7c3aed44;border-radius:6px;padding:4px 10px;">
        <span style="font-size:12px;font-weight:600;color:#a78bfa;">${count} signal${count !== 1 ? "s" : ""}</span>
      </div>
    </div>
...
async function sendDigestDiscord(payload: DigestPayload) {
  const embeds = payload.alerts.slice(0, 10).map(item => {
    ...
    if (item.signals.supports.length > 0)
      fields.push({ name: "Support", value: item.signals.supports.map(s => `$${s.toFixed(2)}`).join(" / "), inline: true });
    if (item.signals.resistances.length > 0)
      fields.push({ name: "Resistance", value: item.signals.resistances.map(r => `$${r.toFixed(2)}`).join(" / "), inline: true });
    ...
    if (item.signals.latestVolume !== null && item.signals.avgVolume20 !== null)
      fields.push({ name: "Volume", value: `${(item.signals.latestVolume / 1_000_000).toFixed(1)}M (${(item.signals.latestVolume / item.signals.avgVolume20).toFixed(1)}x avg)`, inline: true });
  ...
  await fetch(payload.discordWebhook!, {
    ...
    body: JSON.stringify({
      content: `**HLF Wheel Alerts** · ${date} · ${count} signal${count !== 1 ? "s" : ""}`,
      embeds,
    }),
  });
}
