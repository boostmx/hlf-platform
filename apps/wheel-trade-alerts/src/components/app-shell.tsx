...
import { Bell, LayoutDashboard, List, Settings, LogOut, ShieldCheck, TrendingUp, Sun, Moon, Briefcase, Menu, X, Sparkles } from "lucide-react";
    ...
    <>
      ...
      <div className="px-3 pb-4 space-y-0.5 border-t border-sidebar-border pt-3">
        ...
        <div className="flex items-center gap-2 px-3 py-2 mt-1">
          ...
          >
            {mounted ? (resolvedTheme === "dark" ? <Sun className="h-3.5 w-3.5" /> : <Moon className="h-3.5 w-3.5" />) : <div className="h-3.5 w-3.5" />}
          </Button>
          <Button
            variant="ghost"
            size="icon-sm"
      ...
      <aside className="hidden md:flex w-56 border-r border-border bg-sidebar flex-col shrink-0">
        <div className="px-4 py-4 border-b border-sidebar-border">
          <Link href="/dashboard" className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-sm leading-tight">HLF Wheel Alerts</p>
              <p className="text-[10px] text-muted-foreground leading-tight mt-0.5">Wheel strategy signals</p>
            </div>
          </Link>
        </div>
        {sidebarNav}
      ...
      <div className="flex flex-col flex-1 min-w-0">
        ...
        <header className="flex md:hidden items-center gap-3 px-4 py-3 border-b border-border bg-sidebar shrink-0">
          ...
          <Link href="/dashboard" className="flex items-center gap-2 flex-1 min-w-0">
            ...
            <p className="font-semibold text-sm leading-tight truncate">HLF Wheel Alerts</p>
          </Link>
          <Button
            variant="ghost"
            size="icon-sm"
